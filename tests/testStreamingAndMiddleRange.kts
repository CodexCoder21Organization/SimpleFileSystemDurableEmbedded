...
private class ObservedRealBlobstore(private val delegate: InMemoryBlobstoreService) : BlobstoreService by delegate {
...
fun testStreamingAndMiddleRange() {
    ...
    try {
        ...
        try {
            val blobs = ObservedRealBlobstore(InMemoryBlobstoreService())
            ...
            assertEquals(1, blobs.fetchedHashes.size, "A middle range wholly inside block 1 must fetch only that block blob.")
